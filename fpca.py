...
def fpca_pipeline(xgrid, y, nx='all', *args, **kwargs):

    if nx == 'all':
        xgrid = xgrid
    elif type(nx) in [float, int]:
        if 0 < nx <= 1.:
            nx *= xgrid.size
        elif nx <= xgrid.size:
            pass
        else:
            raise ValueError('nx must be in (0, 1] or (1, xgrid.size]')

        choice = np.random.choice(np.arange(len(xgrid)), nx, replace=False)
        sel = np.zeros(xgrid.shape).astype(bool)
        sel[choice] = True

        xgrid = xgrid[sel]
        y = y[:, sel]
    else:
        raise ValueError('nx must be \'all\', or number in allowed range')

    n_diffs, n_channels = y.shape

    X = np.tile(xgrid[:, None], (1, n_diffs))
    Y = y.T

    spline_ridge_model = make_pipeline(
        SplineTransformer(degree=3),
        Ridge())
    spline_ridge_model_paramgrid = dict(
        splinetransformer__n_knots=np.geomspace(2, 20, 10).astype(int),
        ridge__alpha=np.geomspace(1.0e-6, 1.0, 20))

    gridsearch = GridSearchCV(
        spline_ridge_model, spline_ridge_model_paramgrid,
        *args, **kwargs)
    gridsearch.fit(X, Y)
